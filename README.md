...
| 系统 | 支持方式 | CLI Patch | 安装位置 |
|------|---------|-----------|---------|
| macOS | 原生支持 | ✅ npm + ⚠️ 官方安装器（实验性） | 终端直接运行 |
| Linux | 原生支持 | ✅ npm（官方安装器暂不支持） | 终端直接运行 |
| Windows | 通过 WSL | ✅ npm | **必须在 WSL 终端内运行** |

> **Windows 用户**：先安装 [WSL](https://learn.microsoft.com/zh-cn/windows/wsl/install)，然后在 WSL 中安装 Claude Code 和本插件。不支持原生 Windows。

### 安装方式

本插件同时支持两种 Claude Code 安装方式：

| 安装方式 | 说明 | CLI Patch 支持 |
|---------|------|---------------|
| `npm install -g @anthropic-ai/claude-code` | npm 全局安装 | ✅ 完整支持 |
| `curl -fsSL https://claude.ai/install.sh \| sh` | 官方安装器 | ⚠️ 实验性支持（仅 macOS） |

安装脚本会自动检测安装方式，无需手动选择。

> **官方安装器用户注意**：原生二进制 patch 需要 `node-lief` 依赖，运行 `npm install -g node-lief` 安装。如不安装，其他功能（settings、hooks、output styles）仍可正常使用，仅 CLI patch 跳过。
>
> **验证状态**：macOS 官方安装器已在 `2.1.92 → 2.1.101` 升级链路上完成隔离验证；当前口径仍保持为实验性支持。
>
...
- ✅ 在支持的安装方式上 patch 硬编码文字（1463 条翻译，1443 处有效 patch）
...
重启 Claude Code 后，发送任意请求。如果看到 spinner 显示“思考中”、“光合作用中”等中文，说明 Layer 1~3 已生效。若安装脚本还显示了 CLI Patch 成功摘要，则 Layer 4 也已启用。
...
| UI 文字中文化 | 1463 条翻译，1443 处有效 patch | CLI Patch（扫描真实双引号字符串 token 后逐条替换） |
...
├── bun-binary-io.js         ← 原生二进制 I/O 工具（官方安装器支持）
