...
It translates 187 spinner verbs, 41 spinner tips, 139 UI patches (136 translations + 3 special patches for duration/verbs/tips), notification messages, and more. The patch uses per-entry regex matching on double-quoted string boundaries, with regex literal exclusion via offset checks. After Claude Code updates, the plugin automatically re-patches on next session start. Supports macOS, Linux, and Windows (via WSL).
