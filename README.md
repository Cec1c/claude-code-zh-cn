...
>
> **Windows native .exe / latest 不支持 CLI Patch**：Windows native .exe（官方安装器安装的 .exe 二进制，或 `2.1.113+` npm 包的原生 wrapper）目前会明确跳过 CLI Patch，仅启用 Layer 1~3。硬编码 UI 文字不会被完整翻译。如需完整中文化，请使用 `npm install -g @anthropic-ai/claude-code@2.1.112` 安装旧 npm 版本。
>
> **支持边界单一来源**：当前口径以 [docs/support-matrix.md](./docs/support-matrix.md) 为准。该文档部分由 `scripts/upstream-compat.config.json` + `node scripts/verify-upstream-compat.js --json` 生成；Windows 相关行手动维护。
>
> **最新版说明**：Claude Code 从 `2.1.113` 开始，npm 主包切换为 native binary wrapper，不再包含旧的 `cli.js`。本插件当前 stable CLI Patch 支持到 `2.1.112`；macOS 官方安装器旧版本可走 native patch experimental。`2.1.113+` / latest 暂不承诺稳定 CLI Patch。
...
| `npm install -g @anthropic-ai/claude-code` | npm 全局安装最新版 | `unsupported`（`2.1.113+` 已切 native binary wrapper） |
...
| `curl -fsSL https://claude.ai/install.sh \| sh` | 官方安装器 latest | `unsupported`（当前会安装 native binary/latest） |
...
> **官方安装器说明**：官方安装器可以指定旧版本，例如 `curl -fsSL https://claude.ai/install.sh | bash -s 2.1.112`。这个装出来的是 native 二进制，不是旧 npm `cli.js`。本插件已有一套处理方法：用 `bun-binary-io.js` 提取二进制里的 JS → 复用 `patch-cli.sh` 翻译 → 再写回二进制。macOS arm64 `2.1.110`、`2.1.111`、`2.1.112` 已在临时目录验证通过；运行时需要 `node-lief`。latest 仍会被跳过。要最稳，请优先使用 npm pinned 安装方式。
>
...
- ✅ 在支持的安装方式上 patch 硬编码文字（1503 条翻译；当前 stable 代表版本 `2.1.112` 实测 1390 处有效 patch）
...
- 可选：jq（更精准的 JSON 合并）
- 可选：`node-lief`（官方安装器 native experimental 适配需要：`npm install -g node-lief`；当前 stable npm 路径不需要）
...
| UI 文字中文化 | 1503 条翻译，`2.1.112` 实测 1390 处有效 patch | CLI Patch（扫描真实双引号字符串 token 后逐条替换） |
...
├── cli-translations.json    ← 1503 条 UI 翻译对照表
...
注意：Claude Code 从 `2.1.113` 开始，npm latest 切换为 native binary wrapper，不再包含旧的 `cli.js`。当前 stable CLI Patch 支持到 `2.1.112`。官方安装器指定旧版本的 macOS native 二进制已验证到 `2.1.112`，插件会用 native patch experimental 处理；latest 暂不承诺。
</details>
...
It translates 187 spinner verbs, 41 spinner tips, 1503 UI translations, notification messages, and more. The patch combines safe double-quoted string scanning for CLI hardcoded text replacement. On macOS/Linux, the npm launcher self-heals before launch and `session-start` remains as a second fallback. On Windows, a PowerShell install script (`install.ps1`) is available for the old npm `cli.js` form (2.1.92–2.1.112); Windows native `.exe` / `2.1.113+` / latest are detected and skipped for CLI Patch (Layers 1–3 still active). Current support windows are documented in [docs/support-matrix.md](./docs/support-matrix.md).
