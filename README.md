...
| UI 文字全量中文化 | 569 条 | CLI Patch（状态、按钮、错误、设置、导航等） |
...
- 从 `cli-translations.json` 读取 569 条翻译，按字符串长度降序批量替换
...
├── cli-translations.json    ← 569 条 UI 翻译对照表
...
It translates 187 spinner verbs, 41 spinner tips, 569 UI strings (status messages, buttons, errors, settings, navigation), notification messages, and more — all without modifying upstream code. After Claude Code updates, the plugin automatically re-patches on next session start. Supports macOS, Linux, and Windows (via WSL).
