...
- ✅ Patch cli.js 硬编码文字（753 条翻译 + 4 条特殊 patch）
...
| UI 文字全量中文化 | 753 条翻译 + 4 条特殊 patch | CLI Patch（逐条正则匹配双引号字符串） |
...
It translates 187 spinner verbs, 41 spinner tips, 753 UI translations (738 effective patches on Claude Code 2.1.96), notification messages, and more. The patch uses per-entry regex matching on double-quoted string boundaries, with regex literal exclusion via offset checks. After Claude Code updates, the plugin automatically re-patches on next session start. Supports macOS, Linux, and Windows (via WSL).
