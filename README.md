...
| UI 文字全量中文化 | 136 条（134 条生效） | CLI Patch（逐条正则匹配双引号字符串边界） |
...
- 基于 Node.js 的**逐条正则匹配**，对每条翻译构建 `/"...en..."/g` 正则，只在双引号字符串边界内替换
...
It translates 187 spinner verbs, 41 spinner tips, 63 UI strings (status messages, buttons, errors, settings, navigation), notification messages, and more — all without modifying upstream code. The patch uses a JS source segment parser to only replace within double-quoted string literals, never breaking code identifiers. After Claude Code updates, the plugin automatically re-patches on next session start. Supports macOS, Linux, and Windows (via WSL).
